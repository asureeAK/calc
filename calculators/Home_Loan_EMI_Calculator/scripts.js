// scripts.js for Home Loan EMI Calculator

function calculateEMI() {
    const homePrice = parseFloat(document.getElementById('loan-amount').value);
    const downPayment = parseFloat(document.getElementById('down-payment').value) || 0;
    const loanAmount = homePrice - downPayment; // Adjust loan amount based on down payment
    ...
    const totalPayment = emi * tenureInMonths + processingFees + insurance; // Total amount paid
    const totalInterest = (emi * tenureInMonths) - loanAmount; // Total interest paid
